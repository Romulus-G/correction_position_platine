...
# - fait la différence de deux flous gaussiens (flou gaussien = convolution de l'image avec une gaussienne en 2D)
# - applique un flou gaussien sur le résultat (la différence)
# - fait un masque pour prendre que les valeurs assez hautes
...
# image tout en gardant assez pour pouvoir déterminer la translation.
...
if __name__ == "__main__":
    ...
    fig.tight_layout()
    plot_im1 = ax[0].imshow(mask1) ; ax[0].set_title("DoG sur image 1")
    plot_im2 = ax[1].imshow(mask2) ; ax[1].set_title("DoG sur image 2")
    plt.subplots_adjust(bottom=0.25)

    ax_last = plt.axes([0.1, 0.05, 0.25, 0.03])
    ...
    ax_sigma_inf = plt.axes([0.1, 0.11, 0.25, 0.03])
    ...
    ax_sigma_diff = plt.axes([0.1, 0.17, 0.25, 0.03])
    ...
    ax_tol = plt.axes([0.6, 0.05, 0.25, 0.03])
    ...
    ax_ksize = plt.axes([0.6, 0.11, 0.25, 0.03])
    ksize_slider = Slider(
        ax=ax_ksize,
        label='kernel size',
        valmin=3,
    ...
    def update(val):
        ...
        mask2 = DoG(im[1], 
                ...
                (0,0), last_blur_slider.val, 
                threshold_slider.val)
        contours2, hierarchy2 = cv.findContours(mask2, cv.RETR_LIST, cv.CHAIN_APPROX_NONE)
        
        ax[1].set_title(len(contours2))
